...
import { RootToggleAlwaysVisible } from "@/components/root-toggle-always-visible";
import Link from "fumadocs-core/link";
import { LargeSearchToggle, SearchToggle } from "@/components/search-toggle";

import {
  TernSidebarFooter,
  TernSidebarContentMobile,
  TernSidebarPageTree,
} from "@/components/ternsecure-sidebar";
import type {
  SidebarTab,
  SidebarNavigation,
} from "@/components/ternsecure-sidebar";
...
interface SidebarOptions
  ...
  /**
   * TernSecureDocs custom tabs for SDK/framework switching
   */
  tabs?: SidebarTab[];

  /**
   * TernSecureDocs custom navigation structure
   */
...
export function DocsLayout(props: DocsLayoutProps) {
  const {
    ...
    sidebar: { tabs = [], navigation = [], ...sidebarProps } = {},
    i18n = false,
    themeSwitch = {},
  } = props;
  ...
  function ternSecureDocsSidebar() {
    const {
      banner,
      footer,
      collapsible = true,
    ...
    const Footer =
      typeof footer === "function"
        ? footer
        : (props: ComponentProps<typeof TernSidebarFooter>) => (
            <TernSidebarFooter {...props}>
              {props.children}
              {footer}
            </TernSidebarFooter>
          );
    ...
    const rootToggle = (
      <>
        {tabMode === "sidebar" && tabs.length > 0 && (
          <RootToggleAlwaysVisible
            className="mb-2"
            options={tabs as Option[]}
          />
        )}
    ...
    const viewport = (
      <SidebarViewport>
        ...
        <TernSidebarPageTree tabs={tabs} navigation={navigation} />
      </SidebarViewport>
    );
